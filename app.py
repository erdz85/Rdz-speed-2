...
# ==========================================
    ...
    if not st.session_state.workout_logs.empty:
        ...
        for rank_idx, row in leaderboard_df.iterrows():
            ...
            st.markdown(f"""
            <div class="metric-card">
                <b>{medal} {rank_idx+1}. {row['first_name']} {row['last_name']} (Grade {row['grade']})</b>
                <span style="float: right; font-weight: bold;">{score}</span>
            </div>
            """, unsafe_allow_html=True)
...
# ==========================================
    ...
    if not raw_logs.empty:
        ...
        st.info("No recorded sprint logs available for this athlete profile.")
...
# ==========================================
    ...
    for _, athlete in st.session_state.athletes.iterrows():
        ...
        if not a_logs.empty:
            ...
            roster_rows_html += f"""
            <tr>
                <td style="padding: 8px; border-bottom: 1px dashed #ccc;">{athlete['last_name']}, {athlete['first_name']}</td>
                <td style="padding: 8px; border-bottom: 1px dashed #ccc;">{baseline:.2f}s FAT</td>
                <td style="padding: 8px; border-bottom: 1px dashed #ccc; font-weight: bold;">{current_pr:.2f}s FAT</td>
                <td style="padding: 8px; border-bottom: 1px dashed #ccc; color: green;">{net_delta:+.2f}s</td>
                <td style="padding: 8px; border-bottom: 1px dashed #ccc; font-weight: bold;">{proj_100:.2f}s {state_star}</td>
            </tr>
            """
    ...
    <div class="print-document" style="background-color: white; color: black; font-family: monospace; padding: 20px; border: 2px solid black;">
        <div style="text-align: center; border-bottom: 3px double black; padding-bottom: 10px; margin-bottom: 20px;">
            <h2 style="margin: 0; font-weight: bold;">⚡ NORTHSIDE TRACK & FIELD — 2026 SEASON SPEED REPORT</h2>
            <p style="margin: 5px 0 0 0;">Report Generated: {datetime.today().strftime('%B %d, %Y')} | Head Coach: John Doe</p>
        </div>
        
        <div style="border: 1px solid black; padding: 10px; margin-bottom: 15px;">
            <div style="font-weight: bold; background-color: #eaeaea; padding: 3px; margin: -10px -10px 10px -10px; border-bottom: 1px solid black;">[ SQUAD OVERVIEW: VARSITY BOYS ]</div>
            <p style="margin: 5px 0;">• Total Athletes Tracked: {len(st.session_state.athletes)} active roster profiles</p>
        </div>
        
        <div style="border: 1px solid black; padding: 10px; margin-bottom: 15px;">
            <div style="font-weight: bold; background-color: #eaeaea; padding: 3px; margin: -10px -10px 10px -10px; border-bottom: 1px solid black;">📊 PERFORMANCE ROSTER & 100M PROJECTED RANKINGS</div>
            <table style="width: 100%; border-collapse: collapse;">
                <thead>
                    <tr style="border-bottom: 2px solid black; text-align: left;">
                        <th style="padding: 8px;">Athlete Name</th>
                        <th style="padding: 8px;">Baseline Fly</th>
                        <th style="padding: 8px;">Current PR</th>
                        <th style="padding: 8px;">Net Delta</th>
                        <th style="padding: 8px;">Proj. 100m</th>
                    </tr>
                </thead>
                <tbody>
                    {roster_rows_html}
        ...
        <div style="margin-top: 30px; text-align: center; font-size: 0.8rem; border-top: 1px solid black; padding-top: 10px;">
